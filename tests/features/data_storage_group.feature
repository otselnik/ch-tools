Feature: chadmin data-store commands
  ...
  Scenario: Restore recoverable checksums file for broken object storage part.
    When we execute queries on clickhouse01
    """
    DROP DATABASE IF EXISTS restore_recoverable;
    CREATE DATABASE restore_recoverable;
    CREATE TABLE restore_recoverable.checksums_restore (a UInt32, p UInt32) ENGINE=MergeTree() ORDER BY a PARTITION BY p SETTINGS storage_policy='object_storage';
    INSERT INTO restore_recoverable.checksums_restore SELECT number, 0 FROM numbers(3);
    """
    And we remove s3 object for active part file checksums.txt from table restore_recoverable.checksums_restore on clickhouse01
    When we execute command on clickhouse01
    """
    ...
    """
    SELECT count() FROM restore_recoverable.checksums_restore
    """
